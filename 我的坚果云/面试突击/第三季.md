...
**默认数组的长度是16**，数组的长度扩容之后就变为32，会重新对每个hash值进行寻址，也就是用每个hash值跟新数组的 length-1 进行与运算。
...
volatile 是用来解决可见性、有序性的，在有些罕见的条件下，可以有限的保证原子性，他主要`不是用来保证原子性的`。
...
<img src="第三季.assets/image-20220502234029900.png" alt="image-20220502234029900" style="zoom:50%;" />
...
CAS在底层的**硬件级别给你保证一定是原子性的，同一时间只有一个线程可以执行cas**，先比较再设置。其他线程的CAS同时间执行此时会失败。

比如说，在多线程场景的时候，经常可以使用原子类AtomicInteger来替代 synchronized：
...
用该对象当前的值与 var5 进行比较，如果相同，则更新为 var5+var4并且返回true；
...
<img src="第三季.assets/image-20220422173329610.png" alt="image-20220422173329610" style="zoom:50%;" />
...
<img src="第三季.assets/image-20220422173615192.png" alt="image-20220422173615192" style="zoom:50%;" />
...
如果两个线程都是要对同一个位置进行put操作，就采取**CAS**的策略。
...
线程池做的工作主要是 ，处理过程中将任务放入队列。然后在线程创建后启动这些任务，如果线程数量超过了最大数量，超出数量的线程排队等候，等其他线程执行完毕，再从队列中取出任务来执行。
...
- maximumPoolSize：比如设置为 200（maximumPoolSize设置的数量是包含corePoolSize的）。当队列中一下子来了很多任务，corePoolSize 这3个线程也还在处理任务，没有空闲的，这时就会再创建新的线程，能创建的最大数量就是这个maximumPoolSize；
...
如果超出最大承载（额外线程也满了，队列也满了），此时还有任务要进来，只能reject 掉，他有几种 reject 策略，可以传入 RejectedExecutionHandler。

![image-20220506100104247](第三季.assets/image-20220506100104247.png)

#### 线程池的4种拒绝策略

比如银行的窗口人满了，候客区也满了，这时再有人想办理业务，就会启用拒绝策略。

RejectedExecutionHandler handler 有四种拒绝策略：

1. new ThreadPoolExecutor.AbortPolicy()  =》直接抛异常，这个是默认的。
2. new ThreadPoolExecutor.DiscardPolicy() // 队列满了，尝试和最早的去竞争，也不会抛出异常。 
...
1. FixedThreadPool和SingleThreadPool，使用的队列是LinkedBlockingQueue，允许的请求队列的最大长度为Integer.MAX_VALUE，可能会堆积大量的请求，从而导致OOM；
2. CachedThreadPool和ScheduleThreadPool，允许创建的最大线程数量为 Integer.MAX_VALUE，可能会创建大量的线程，从而导致OOM。

### 工作中线程池配置合理线程数

1、查看一下 运行的阿里云或者服务器是几核的：
...
CPU密集型（比如代码里边写的 while 循环这种）： 意思是该任务需要大量的运算，而没有阻塞，CPU一直全速运行。

CPU密集型任务配置尽可能减少线程数量，一般公式： CPU核数+1个线程的线程池。

#### IO密集型

IO密集型（比如有很多任务，需要不停去数据库/Redis里去拿数据，有IO操作）：由于IO密集型任务线程并不是一直在执行任务，则应配置尽可能多的线程，如CPI核数 * 2。

<img src="第三季.assets/image-20220511113004956.png" alt="image-20220511113004956" style="zoom:50%;" />
...
如果系统资源充足，进程的资源请求都能够得到满足，死锁出现的可能性就很低，否则，就会因争夺有限的资源而陷入死锁。

<img src="第三季.assets/image-20220511113830261.png" alt="image-20220511113830261" style="zoom:50%;" />

#### 你能手写个死锁代码吗？

详见：/Users/lyf/Workspace/www/blog-demo/thread-demo/src/main/java/lock/DeadLock.java
...
```bash
@MacBook-Pro   ~/Workspace/www/blog-demo/thread-demo/src/main/java/lock   master ●✚  jps -l              
3349 sun.tools.jps.Jps
99657 lock.DeadLock
99658 org.jetbrains.jps.cmdline.Launcher
12335 
```
...
```bash
@MacBook-Pro   ~/Workspace/www/blog-demo/thread-demo/src/main/java/lock   master ●✚  jstack 99657 
......
Found one Java-level deadlock:
=============================
"T2":
  waiting to lock monitor 0x00007fdaf8013018 (object 0x00000007956a7250, a java.lang.String),
  which is held by "T1"
"T1":
  waiting to lock monitor 0x00007fdaf8015698 (object 0x00000007956a7288, a java.lang.String),
  which is held by "T2"

Java stack information for the threads listed above:
===================================================
"T2":
        at lock.MyDeadLock.run(DeadLock.java:44)
        - waiting to lock <0x00000007956a7250> (a java.lang.String)
        - locked <0x00000007956a7288> (a java.lang.String)
        at java.lang.Thread.run(Thread.java:748)
"T1":
        at lock.MyDeadLock.run(DeadLock.java:44)
        - waiting to lock <0x00000007956a7288> (a java.lang.String)
        - locked <0x00000007956a7250> (a java.lang.String)
        at java.lang.Thread.run(Thread.java:748)

Found 1 deadlock.
```

发现会有这些输出，就证明了`这两个线程产生了死锁`。
...
JMM就作用于工作内存和主内存之间数据同步过程，**他规定了如何做数据同步以及什么时候做数据同步。**
...
JMM是一种规范，目的是解决由于多线程通过共享内存进行通信时，存在的本地内存数据不一致、编译器对代码执行重排序、处理器会对代码乱序执行带来的问题。

#### JMM诞生原因

Java为了保证并发编程中可以满足原子性、可见性及有序性，诞生了一个重要的概念，那就是Java内存模型。

内存模型 定义了**共享内存系统中多线程程序读写操作行为的规范**。通过这些规则来规范对内存的读写操作，从而保证指令执行的正确性，它解决了 CPU 多级缓存、处理器优化、指令重排等导致的内存访问问题 ， **保证了并发场景下的一致性、原子性和有序性** 。
...
ack:1, syn = 1,ack num = x+ 1,seq =y;
ack:1, ack num = y+1, seq = x+1;
...
尚硅谷阳哥：https://www.bilibili.com/video/BV1zb411M7NQ?p=57&spm_id_from=pageDriver

#### Java类加载过程中的沙箱安全机制？

 oracle收购了两家JVM（两套jdk规范，一个是Jrocket，一个是Hotspot）的公司，将永久代上面那一层干掉了，干掉，  替而代之的是 元空间。

<img src="第三季.assets/image-20220511153733377.png" alt="image-20220511153733377" style="zoom:50%;" />

#### GCRoots

<img src="第三季.assets/image-20220511155400673.png" alt="image-20220511155400673" style="zoom:50%;" />

从这四个起点出发，做为GCRoot，去遍历对象，能够串起来，就叫对象可达，这种情况下就不可以回收。

<img src="第三季.assets/image-20220511160057295.png" alt="image-20220511160057295" style="zoom:50%;" />

第4种，比如：Thread的start方法。

以这种四种对象做为顶点，自顶向下，找到完整的引用链，就叫做引用可达。

### JVM的两个经典参数

-Xms和-Xmx。

比如说：-Xms1024m -Xmx1024m。
...
<img src="第三季.assets/image-20220511180028984.png" alt="image-20220511180028984" style="zoom:50%;" />

#### 软引用

内存够用的时候保留，不够用就回收。

<img src="第三季.assets/image-20220511180700791.png" alt="image-20220511180700791" style="zoom:50%;" />



#### 弱引用

不管内存是否够用，只要垃圾回收机制一运行，不管JVM的内存空间是否足够，都会回收该对象占用的内存。

<img src="第三季.assets/image-20220511181142165.png" alt="image-20220511181142165" style="zoom:50%;" />

#### 软引用和弱引用的使用场景

假如说现在要读取大量的图片。 

<img src="第三季.assets/image-20220511181542322.png" alt="image-20220511181542322" style="zoom:50%;" />

Mybatis缓存的那块大量使用了软引用。