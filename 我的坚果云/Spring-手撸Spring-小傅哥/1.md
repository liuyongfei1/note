...
new关键字和newInstance()的区别见：/Users/lyf/Workspace/www/blog-demo/test-demo/src/main/java/com/fullstackboy/reflection/Demo1.java
...
=》总结补充：是在上一章的基础上，createBean方法里createBeanInstance（实例化bean）后，添加了`applyPropertyValues（给Bean填充属性）`方法。
...
`1. 如果 value 是  Object ，则要继续调 getBean方法，其实就是递归创建 Bean 和填充属性。`

2. BeanUtil.setFieldValue 是调用的 hutool 包封装好的方法。
...
借助hutool jar包里的`XmlUtil.readXML`方法:
...
- JDK动态代理是通过实现目标类的接口，然后将目标类在构造动态代理类时做为参数传入，使代理对象持有目标对象，再通过代理对象的	实现动态代理的操作；
...
满足于对 Bean 对象扩展的两个接口，其实也是 Spring 框架中非常具有重量级的两个接口：`BeanFactoryPostProcessor 和 BeanPostProcessor`，也几乎是大家在使用 Spring 框架外新增开发自己组件需求的两个必备接口。
...
比如 spring中，如果没有应用上下文，你还得写

- 读取xml配置文件&注册bean的代码；
- 将自己写的扩展点，比如MyBeanFactoryPostProcessor、MyPostProcessor  生效，执行扩展逻辑。

详细见：/Users/lyf/Workspace/www/small-spring/small-spring-step-06/src/test/java/cn/bugstack/springframework/test/ApiTest.java
...
- 实现BeanFactoryPostProcessor接口，可以在spring的bean创建（其实就是**实例化bean**，`对应AbstractApplicationContext#refresh里的最后一步：beanFactory.preInstantiateSingletons()`，就是利用动态代理实例化bean）之前，修改bean的定义属性；=》`对应 AbstractApplicationContext#refresh方法里的 invokeBeanFactoryPostProcessors()方法`  => 20220423 12:20 备注
...
- 可以在spring容器**实例化bean**之后，在执行bean的初始化方法前后，添加一些自己的处理逻辑
   `对应AbstractApplicationContext#refresh里的最后一步：beanFactory.preInstantiateSingletons()`

  继续追踪代码，查看调用链 =》 AbstractAutowireCapableBeanFactory#createBean：
  ...
  private Object initializeBean(String beanName, Object bean, BeanDefinition beanDefinition) {
          // 1. 执行 BeanPostProcessor Before 处理 《= 执行初始化方法之前
...
就是在 initializeBean 方法中，在 invokeInitMethods 方法之前新增：
...
那么我们就需要先来实现一个可以代理方法的 Proxy，其实**代理方法主要是使用到 方法拦截器类处理方法的调用 MethodInterceptor#invoke，而不是直接使用 invoke 方法中的入参 Method method 进行 method.invoke(targetObj, args) 这块是整个使用时的差异**。 
...
4. 查看结果，是否成功拦截方法：
   ...
   ReflectiveMethodInvocation 就是一个入参的包装信息，提供了目标对象、方法、入参。
...
我们需要在本章完成 AOP 核心功能与 Spring 框架的整合，最终能通过在 Spring 的配置方式完成切面的操作。
...
*DefaultAdvisorAutoProxyCreator*，**这个类就是用于处理AOP 代理融入到 Bean 生命周期中的核心类。**

主要核心实现：postProcessBeforeInstantiation方法。
...
关键的就是这个 **doLoadBeanDefinitions** 方法。=》见下文。
...
其中，新增的 PropertyPlaceholderConfigurer 就是实现了 BeanFactoryPostProcessor接口：
...
这就像我们使用 Spring 框架中 @Autowired、@Value 注解一样，完成我们对**属性和对象**的注入操作。

比如，上一章节中，我们需要在xml配置文件中配置：
...
本章节，我们将 通过 property name="token" 配置属性 改为可以自动注入。也就是说在xml中不用再配置：
...
- 所有关于 Bean 等等的操作，以及监听 Aware、获取 BeanFactory，都需要在 Bean 的生命周期中完成。
...
public PropertyValues postProcessPropertyValues(PropertyValues pvs, Object bean, String beanName) throws BeansException {
    ...
    for (Field field : declaredFields) {
        ...
        if (null != valueAnnotation) {
            ...
            // 利用字符串解析器来解析占位符：
            // PropertyPlaceholderConfigurer#postProcessBeanFactory 会加载属性文件(在 Bean 实例化之前，执行 BeanFactoryPostProcessor的 invokeBeanFactoryPostProcessors方法时调用这个postProcessBeanFactory方法。)
            // 
    ...
    for (Field field : declaredFields) {
        ...
        if (null != autowiredAnnotation) {
            ...
            if (null != qualifierAnnotation) {
                ...
                // 如果这个属性（比如 userDao）加了 @Autowired 注解，则 创建这个 bean
...
可重点关注 postProcessBeanFactory、resolvePlaceholder 方法。
...
```txt
AbstractAutowireCapableBeanFactory#initializeBean ->执行完 Bean 对象的初始化方法(invokeInitMethods) 

-> 执行：AbstractAutowireCapableBeanFactory#applyBeanPostProcessorsAfterInitialization

-》DefaultAdvisorAutoProxyCreator#postProcessAfterInitialization 创建代理对象
```
...
```java
AbstractAutowireCapableBeanFactory#createBean ->
  applyBeanPostProcessorsBeforeApplyingPropertyValues(在设置 Bean 属性之前，允许 BeanPostProcessor 修改属性值，修改后 "token" = "RejDlI78hu223Opo983Ds")
  -》applyPropertyValues (给 Bean 填充属性)
...
三级缓存其实就是三个 Map。（参考自：https://www.cnblogs.com/semi-sub/p/13548479.html）

<img src="1.assets/image-20220307094429018.png" alt="image-20220307094429018" style="zoom:50%;" />
...
-> // 8. 提前实例化单例Bean对象
        beanFactory.preInstantiateSingletons();
-> (从 beanDefinitionMap 循环创建 bean)，getBean()
-> doGetBean()
-> createBean()
-> doCreateBean()
...
`下面是重点，我们会发现这个二级缓存好像显得有点多余，好像可以去掉，只需要一级和三级缓存也可以做到解决循环依赖的问题？`

拿单元测试的代码进行debug，当 bean 是 "wife" 时，会执行 
...
我们来详细看一下这个 wrapIfNecessary() 方法：

```java
protected Object wrapIfNecessary(Object bean, String beanName) {
  // 如果是切面相关的类，比如 Advice、PointCut、Advisor，则直接返回
  if (isInfrastructureClass(bean.getClass())) return bean;
  ...
  for (AspectJExpressionPointcutAdvisor advisor : advisors) {
    ClassFilter classFilter = advisor.getPointcut().getClassFilter();
    // 过滤匹配类(比如，下面的单元测试中，"husbandMother" 这个bean 虽然能走到这里，但是由于跟切面表达式不匹配，所以跳过本次循环)
    // 然后，当 bean 是 "wife" 时，能够匹配，则继续执行下面的代码，产生一个新的代理对象，且该代理对象被AOP 进行切面了。
    if (!classFilter.matches(bean.getClass())) continue;
...
我们可以拿单元测试来进行验证上面的结果，会发现：

- 当 bean 是 "husbandMother"时，与切面表达式匹配不成功，直接返回 bean 对象；
- 当 bean 是 "wife"时，与切面表达式匹配成功，会执行 new ProxyFactory(advisedSupport).getProxy()，

回到 getSingleton方法，第一次执行后，singletonObject为：

![image-20220308154842994](1.assets/image-20220308154842994.png)我们再次执行 singletonFactory.getObject()，会发现又新创建了一个代理对象：

<img src="1.assets/image-20220308155108334.png" alt="image-20220308155108334" style="zoom:50%;" />



##### 总结

**1、因为 Bean 是单例的，所以这里我们要借助二级缓存来解决这个问题。将执行了 singletonFactory.getObject() 的对象放到二级缓存中去，后面去二级缓存中拿，没必要再执行一遍 singletonFactory.getObject() 方法再产生一个新的代理对象，保证始终只有一个代理对象。**

**2、所以如果没有 AOP的话，确实两级缓存可以解决循环依赖的问题，如果加上 AOP，两级缓存是无法解决的，不可能每次执行 singletonFactory.getObject() 方法都给我产生一个新的代理对象，所以还要借助另外一个缓冲来保存产生的代理对象。**
