...
由于在xml文件应用上下文创建的时候，"proxyUserDao"这个bean已经放到了 factoryBeanObjectCache 这个Map里去，所以这里object不为空，会直接返回object。
...
- 也就是说这里 通过 ProxyBeanFactory 对象调用它的getObject方法生成 IUserDao 的动态代理类，做为userService 这个bean的userDao属性值；
- 当单元测试类里
