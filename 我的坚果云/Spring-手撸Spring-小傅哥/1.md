...
#### 目标
> 1、在集合Spring框架下，我们使用的Mybatis框架中，它的核心作用是可以满足用户不需要事先Dao接口类，就可以通过xml或注解配置的方式完成对数据库执行CRUD操作。
>
...
```java
 doCreateBean(beanName, beanDefinition, args);
```
...
<img src="1.assets/image-20220223133459673.png" alt="image-20220223133459673" style="zoom:50%;" />

**此时，converters 里 存入了一个 StringToLocalDateConverter。**

还有一个 StringToNumberConverterFactory 是什么时候 存入的呢？
...
protected <T> T doGetBean(final String name, final Object[] args) {
  ...
  BeanDefinition beanDefinition = getBeanDefinition(name);
  Object bean = createBean(name, beanDefinition, args);
  // 执行完上面这一行converters 还是只有StringToLocalDateConverter
  // 因此主要是这一行
  return (T) getObjectForBeanInstance(bean, name);
}
```
...
<img src="1.assets/image-20220223141846100.png" alt="image-20220223141846100" style="zoom:50%;" />

到ConversionServiceFactoryBean#getObject 方法里时，发现就已经是两个了。

<img src="1.assets/image-20220223141749500.png" alt="image-20220223141749500" style="zoom:50%;" />
